...
class BD(BDRegMat):
        ...
        try:
            ...
            for line in f:
                ...
                if (len(liste)==2):
                    ...
                    # Q5 ...
                    self.requete("INSERT INTO lieux VALUES({0}, '{1}', 'Calvodos')".format(liste[0], liste[1]))
            f.close()
        except IOError as e:
    ...
    def personneCode(self,ident):
        ...
        try:
            ...
            # Q9 ...
            table = self.question("SELECT sexe, nom, prenom, profession, dateNaissance, nomCommune, personnes.codeINSEE"+
                                  "FROM personnes, lieux"+
                                  "WHERE ident = {0} AND personnes.codeINSEE == lieux.codeINSEE".format(ident))
            if (len(table)>0):
                if (len(table)==1):
    ...
    # Q10: c'est la méthode personneMiseAJour qui permet de procéder à la mise à jour l'onglet Personne
    #       et la méthode personneInsertion déclenche l'appel
    ...
    def listeMilitaires(self):
        ...
        try:
            # Recherche des militaires
            # Q11 ...
            table = self.question("SELECT ident, prenom, nom, dateNaissance, nomCommune"+
                                  "FROM personnes, lieux"+
            ...
            if (len(table)>0):
                ...
                for personne in table:
                    # ...
                    self.out.afficher("{0}: M.{1} {2}, né le {3} à {4}".format(personne[0], personne[1], personne[2], personne[3], personne[4]))
            else:
    ...
    def listeResidences(self):
        ...
        try:
            ...
            # Q13 ...
            table = self.question("SELECT ident, prenom, nom, nomCommune, date"+
                                  "FROM personnes, lieux, resider"+
                                  "WHERE resider.ident == personnes.ident AND resider.codeINSEE == lieux.codeINSEE")
            if (len(table)>0):
                self.out.afficher("Nombre de résidences connues : {0}\n".format(len(table)))
                # Affichage de chaque lieu
                for lieu in table:
                    # ...
                    self.out.afficher("{0}: M.{1} {2} réside à {3} le {4}".format(lieu[0], lieu[1], lieu[2], lieu[3], lieu[4]))
            else:
