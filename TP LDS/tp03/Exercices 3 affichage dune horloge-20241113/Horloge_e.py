  ...
  def affichageChiffre(self,ch,base):
    for i in range(Horloge.NBSEG):
      ...
      if Horloge.SEGMENTS_CHIFFRES[ch][i]==1:
        self.image.setColor(base+i, qRgb(255, 255, 255)) # Q5: ici, modifie de self.gr à 255 car self.gr = 30 (gris)
      else:
        self.image.setColor(base+i, qRgb(self.gr, self.gr, self.gr))

  # Gestion du lancement de l'horloge
  def majHorloge(self):
    # Q3: C'est ici où on récupère 6 chiffres de l'heure courante
    temps=str(datetime.now().time())
    # 17:40:49.030946
    # Q4: Pour limiter les mises à jour inutiles -> performance plus efficace
    #     Les secondes changent plus fréquemment que les minutes et les heures
    #     Interruption du balayage si les chiffres n'ont pas changé: self.cp[j]!=self.c[j]
