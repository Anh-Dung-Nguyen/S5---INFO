    ...
    def floyd(self):
        ...
        for y in range(self.image.height()-1):
            ...
            for x in range(1,self.image.width()-1):
                ...
                nvv1=max(0,min(fille.image.pixelIndex(x+1,y)+7*err/16,255),0)
                fille.image.setPixel(x + 1, y, int(nvv1))
                # A compléter pour diffuser l'erreur sur les points de l'image fille...
                nvv2=max(0,min(fille.image.pixelIndex(x-1,y+1)+3*err/16,255),0)
                fille.image.setPixel(x - 1, y + 1, int(nvv2))
                nvv3=max(0,min(fille.image.pixelIndex(x,y+1)+5*err/16,255),0)
                fille.image.setPixel(x, y + 1, int(nvv3))
                nvv4=max(0,min(fille.image.pixelIndex(x+1,y+1)+1*err/16,255),0)
                fille.image.setPixel(x + 1, y + 1, int(nvv4))
    ...
    def bitPoidsFaible(self,n):
        for i in range(FenetreFilleTC.TAILLE_TDC):
            ...
            if (i % 2 == 0):  
                ...
                self.image.setColor(i, qRgb(255, 255, 255))
        # Recopie de l'image avec la nouvelle table des couleurs dans la fenêtre
        self.recopieImage(self.image)
        return(self)
    ...
    def mystere(self):
        for i in range(FenetreFilleTC.TAILLE_TDC):
            ...
            if (i % 2 == 0):  
                ...
                self.image.setColor(i, 255 - qRgb(255, 255, 255))
            else:
                self.image.setColor(i, qRgb(255, 255, 255))
        # Recopie de l'image avec la nouvelle table des couleurs dans la fenêtre
        self.recopieImage(self.image)
