...
class EAN13:
    ...
    def verification(self):
        # Q2: Cette ligne de code vérifie si dans self.code contient les caractères non numériques et si oui, il retourne False
        if re.compile(r"\D").search(self.code)!=None:
            return(False)
        elif len(self.code) == len(self.LGEAN13) - 1:
            self.code += self.cle()
            return True
        elif len(self.code) == len(self.LGEAN13):
