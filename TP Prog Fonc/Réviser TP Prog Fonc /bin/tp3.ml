...
type carte = Carte of haut * coul (* remplacer par votre type *)
...
let random_carte () = carte (Random.int 7 + 7) (match Random.int 3 with
																								|0 -> "Coeur"
																								|1 -> "Trefle"
																								|2 -> "Pique"
																								|_ -> "Carreau")
...
let rec ajtcarte l = 
	let c = random_carte () in
		if List.mem c l then ajtcarte l
		else c :: l
...
let rec reussite l = if (reduc l) = l then l
											else reussite (reduc l)
...
let draw_reussite () = let jeu = List.map (fun l -> [l]) (faitjeu 32) in 
																	...
																	let rec loop j = 
																		let a = read_key () in
																			if a = 'q' then Graphics.close_graph () 
																			else let k = reduc j in
