...
(* Compléter *)
let rec sigma (a,b) = if a > b then 0
                      else a + sigma(a + 1, b)
...
(* Compléter *)
let rec sigma2 f (a,b) = if a > b then 0
...
(* Compléter *)
let rec sigma3 (f,fc) i acc (a,b)  = if a > b then acc
...
(* Compléter *)
let rec sigma4 (f,fc) (p,fi) acc a  = if p a then acc
...
(* Compléter *)
let cum f (a,b) dx = sigma4 (f, (fun c d -> c +. d)) ((fun v -> v > b), (fun v -> v +. dx)) 0. a
...
(* Compléter *)
let rec maxi f (a,b) p = if abs_float (a -. b) < p then f a
                          else let x = a +. 0.3 *. (b -. a) in
                                let y = a +. 0.6 *. (b -. a) in
                                  if f x < f y then maxi f(x, b) p
                                  else maxi f (a, y) p
